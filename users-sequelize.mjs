...
export async function update( username, password, provider, familyName, givenName, middleName, emails, photos ) {
    const user = await SQUser.findOne( username );
...
export async function find( username ) {
    ...
    // const datiUser = user[0].dataValues
    const ret = user ? sanitizedUser( user ) : undefined;
    log( 'find return User' + util.inspect( ret ) );
    return ret;
}
...
export function sanitizedUser( user ) {
    log( 'user da sanitizer mail' + util.inspect( user.emails ) );
    log( 'user da sanitizer username' + util.inspect( user.username ) );
    var ret = {
